...
public class TLS
{
    ...
    public void Parse(byte[] data)
    {
        ...
        //handshake length
        int handshake_length = Utils.BytesToUShort(new byte[] { data[3], data[4] });
        Console.WriteLine($"(offset: 3)\tHandshake length (bytes): {data[3]:X2}{data[4]:X2} (int : {handshake_length}) handshake ends at row {(handshake_length + 4) / 16} col {(handshake_length + 4) % 16})");
        //handshake header
        Console.WriteLine($"(offset 5)\tHandshake Type: {data[5]:X2} (1 == Client Hello))");
        Console.WriteLine($"(offset 6)\tHandshake length (bytes): {data[6]:X2}{data[7]:X2}{data[8]:X2} (int : {Utils.UInt24ToUInt32(new byte[] { data[6], data[7], data[8] })})");
        //Client Version
        ...
        if (data[9] == 0x03 && data[10] == 0x03)
        {
            ...
            ParseTLS_1_3(data, handshake_length);
            ParseTLSv1_3(data);
    ...
    private void ParseTLS_1_3(byte[] data, int handshake_length)
    {
        ...
        Console.WriteLine($"(offset {offset} (riga {row}, col {col}))\tRandom (32 bytes): 0x{BitConverter.ToString(clientRandom).Replace("-", " 0x")}");
        ...
        Console.WriteLine($"(offset {offset} (riga {row}, col {col}))\tSession ID size: {data[offset]:X2} (int : {sessionID_size})");
        fakeSessionID = new byte[sessionID_size];
        Array.Copy(data, offset + 1, fakeSessionID, 0, sessionID_size);
        Console.WriteLine($"(offset {offset} (riga {row}, col {col}))\tSession ID: {BitConverter.ToString(fakeSessionID).Replace("-", " ")}");
        offset += 1 + sessionID_size;
        //Cipher Suites
        col = offset % 16;
        row = offset / 16;
        ushort cipher_suites_length = Utils.BytesToUShort(new byte[] { data[offset], data[offset + 1] }); //length in bytes
        Console.WriteLine($"(offset {offset} (riga {row}, col {col}))\tCipher Suites Length: {data[offset]:X2} {data[offset + 1]:X2} (int : {cipher_suites_length / 2})");
        offset += 2;
        col = offset % 16;
        row = offset / 16;
        for (int i = 0; i < cipher_suites_length; i += 2)
        {
            Console.Write($"(offset {offset} (riga {row}, col {col}))\tCipher Suite: 0x{data[offset]:X2}, 0x{data[offset + 1]:X2} -> ");
        ...
        Console.WriteLine($"(offset {offset} (riga {row}, col {col}))\tCompression Data: {data[offset]:X2} {data[offset + 1]:X2}");
        offset += 2;
        //Extensions
        col = offset % 16;
        row = offset / 16;
        ...
        //same as TLS 1.2
        Console.WriteLine($"(offset {offset} (riga {row}, col {col}))\tExtensions Length: {data[offset]:X2} {data[offset + 1]:X2} (int : {extensions_length}))");
        offset += 2;
    ...
    private void ParseTLS_UP_TO_1_2(byte[] data, int handshake_length)
    {
        ...
        Console.WriteLine($"(offset 44 (riga {44 / 16}, col {44 % 16}))\tCipher Suites Length: {data[44]:X2} {data[45]:X2} (int : {cipher_suites_length / 2})");
        int offset = 46;
        int col = offset % 16;
        int row = offset / 16;
        ...
        {
            offset += i;
            Console.Write($"(offset {offset} (riga {row}, col {col}))\tCipher Suite: 0x{data[46 + i]:X2}, 0x{data[47 + i]:X2} -> ");
        ...
        col = offset % 16;
        row = offset / 16;
        Console.WriteLine($"(offset {offset} (riga {row}, col {col}))\tCompression Methods Length: {data[offset]:X2} {data[offset + 1]:X2} (int : {Utils.BytesToUShort(new byte[] { data[offset], data[offset + 1] })})");
        offset += 2;
        col = offset % 16;
        row = offset / 16;
        //Extensions length
        int extensionsLength = Utils.BytesToUShort(new byte[] { data[offset], data[offset + 1] });
        Console.WriteLine($"(offset {offset} (riga {row}, col {col}))\tExtensions Length: {data[offset]:X2} {data[offset + 1]:X2} (int : {extensionsLength}))");
        offset += 2;

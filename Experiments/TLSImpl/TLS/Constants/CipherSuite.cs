...
{
    public static byte[] GetBytes(Ciphers cipherName)
    {
        return Utils.UShortToBytes((ushort)cipherName);
    }

    //get cipher by byte array
    public static Ciphers GetCipher(byte[] data)
    {
        if (data.Length != 2)
        {
            throw new Exception("data length is not 2");
        }
        ushort cipherName = Utils.BytesToUShort(data);
        return (Ciphers)cipherName;
    }
